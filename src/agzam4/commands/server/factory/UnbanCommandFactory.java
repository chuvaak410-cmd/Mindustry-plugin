package agzam4.commands.server.factory;

import agzam4.commands.server.UnbanCommand;
import agzam4.commands.factory.ICommandLeafFactory;
import agzam4.commands.CommandHandler;

/**
 * Constructs the {@link UnbanCommand} leaf command. Pure indirection: identical
 * to the original {@code new UnbanCommand()} call site, just reached through a
 * factory so the command is registry-discoverable.
 */
public class UnbanCommandFactory implements ICommandLeafFactory {

	@Override
	public CommandHandler<?> createCommand() {
		return new UnbanCommand();
	}

	@Override
	public String commandName() {
		return "UnbanCommand";
	}
}
